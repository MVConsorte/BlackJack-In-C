...
#define NUM_ACTIONS 5 //Usado na função action_player() -> define número de ações possíveis
...
typedef struct {
    ...
    Card hand[12]; //número máximo de cartas na mão (12: considerando a melhor das hipotese)
...
float bet(Player *user, float prior_bid);  //função que retorna um valor de aposta, além de atualizar conta do usuário #contém dialogo

int action_player(Player *user, int key_option); //função que retorna a ação escolhida pelo usuário
...
void blackJack_game(Player *banca, Player *jogador); //função propriamente do jogo
...
int main(){
    ...
    Player jogador = {"Smith", 100, {}, 0}, dealer = {"John",2000, {}, 0};
...
float bet(Player *user, float prior_bid) { //prior bid = lance prévio
    float aposta;

    if (prior_bid > 0){ //se houver entrada de volor prévio => double down (2 x valor previo)
        if(user->coin >= prior_bid){
            aposta = 2 * prior_bid;
            user->coin -= prior_bid; //retirar da conta do usuario o adicional
            printf("\n\t\tAposta foi dobrada.\n\t\tValor: %.2f coin.\n\n", aposta);
            sleep(2);
            return aposta;
        } else {
            printf("\n\t\tSaldo insuficiente para dobrar aposta.\n\n");
            sleep(2);
            aposta /= 2; //resetar incremento da aposta
            return -1;
        }
    }

    while (1) { // 1 == true
        printf("Faça sua aposta (zero ou negativo para cancelar): ");
        if (scanf("%f", &aposta) != 1 || aposta <= 0 || aposta > user->coin) {
            if (aposta <= 0) {
                printf("Aposta cancelada.\n");
                // aposta cancelada -> custo de render-se? ou retorna ao menu?
                return 0; // Retorna 0 para indicar aposta cancelada
            } else {
                printf("Aposta inválida. Tente novamente.\n");
                while(getchar() != '\n'); //limpar buffer
                clear_terminal();//limpar mensagem de erro
            }
        } else {
            user->coin -= aposta;
...
int action_player(Player *user, int key_option){
    /* 5 ações possíveis:
        Hit ? pedir mais cartas.
        Stand? Não tirar mais cartas.
        Double down ? dobre sua aposta original.
        Split ? divida sua mão em duas se as duas cartas forem um par idêntico.
        Surrender ? retire-se após o crupê distribuir suas cartas iniciais.
    */
    ...
    //Pensando no momento antes de entrar no jogo
    if (key_option){ 
        while (1){
            printf("Saldo: %.2f\n___________________\n", user->coin);
            ...
            if (scanf("%d", &choice) != 1 || choice < 1 || choice > 2){
                ...
                return (choice == 1) ? 5 : 0; // 5 == surrender && 0 == Hit
            }
        }
    }

    // escolheu entrar no jogo
    while(1){
        printf("\nEscolha sua ação:\n");
        ...
        if (scanf("%d", &choice) != 1 || (choice < 0 || choice >= NUM_ACTIONS)){
            while(getchar() != '\n'); //limpar buffer
        ...
        // foi validado a informação, então saia do loop
        break;
    }
    return choice;
...
int count_points(Card *hand, int num_cards) {
    ...
    // Calcular a pontuação da mão, contando Áses e somando outros valores
    for (int i = 0; i < num_cards; i++) {
        if (!strcmp(hand[i].face, "A")) {
            num_aces++;
            score += 11; // Considerar o ás como 11
        } else {
            score += hand[i].value;
        }
    }

    // Ajustar o valor dos áses se necess?rio
    while (score > 21 && num_aces > 0) {
        score -= 10; // Reduzir o valor de um ás de 11 para 1
        num_aces--;
    }
...
int round_21(Player *dealer, Player *user){
    ...
    Finalizada a rodada do jogador, segue a ação do dealer:
        ...
        - compara os valores e retorna: vitória, derrota ou empate
        - dado o resultado, retorna a aposta ao jogador, subtraindo do dealer
        - se o dealer chegar a 0 ou menos, retornar para o main: vitória (Quebrou a banca!), derrota (Saiu de mãos abanando!) ((talvez pausa))]
    ...
    */
    ...
    //Ações do jogador ***************************
    int action, first_motion = 1; // ação && condições de primeiro acesso (para split e double down)
    float stake = 0;

    while(1){
        action = action_player(user, 1); // Perguntar se desistirá
    ...
    while (score_user <= 21){
        clear_terminal(); //limpar prompt
        action = action_player(user,0);
        if (action == 1){ //stand
            score_user = count_points(user->hand, user->num_cards); // update score user
            casino_table(0, dealer, user);
            break;
        }
        if (action == 0){//hit
        ...
        if (action == 2 && first_motion){//Double down
            ...
            if(stake == -1) continue; // Se não há saldo suficiente -> reset 
            give_card(deck,&size_deck, user->hand, &user->num_cards);
            score_user = count_points(user->hand, user->num_cards);
            casino_table(0, dealer, user);
            break;
        ...
        if (action == 3 && first_motion){//split  //**** em manuten??o ****
...
void rounds(Player *dealer, Player *user){
    ...
    while (user->coin > 0 && dealer->coin > 0 && !stop_key){
        ...
        while(1){
            ...
            if((scanf("%d", &stop_key) == 1)){
                if(stop_key == 1 || stop_key == 2) break;
            }
            while(getchar() != '\n'); // limpar buffer
            clear_terminal();
        }
